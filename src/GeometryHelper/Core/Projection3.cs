...
namespace GeometryHelper.Core
{
    ...
    public static class Projection3
    {
        ...
        /// <summary>
        /// Finds the shortest segment connecting two line segments, reading either or both as the infinite
        /// line carrying it, within a tolerance.
        /// </summary>
        /// <param name="line1">The first segment; the result starts on it, or on the line carrying it when extended.</param>
        /// <param name="line2">The second segment; the result ends on it, or on the line carrying it when extended.</param>
        /// <param name="extension">Which of the two may be reached past its endpoints.</param>
        /// <param name="tolerance">The tolerance used to detect degenerate segments.</param>
        /// <returns>The shortest segment between the two, as far as each is allowed to reach.</returns>
        /// <remarks>
        /// With <see cref="LineExtension.Both"/> this is Tekla's <c>Intersection.LineToLine</c>: two axes that
        /// pass each other in space are joined by their common perpendicular, whose length is the gap between
        /// them. Where every pair of facing points is equally close, as for parallel lines, one end of the result
        /// sits on a start point: that of the first segment, unless only the first is extended, and then that of
        /// the second.
        /// </remarks>
        ...
        public static GeoLine3 GetClosestSegment(GeoLine3 line1, GeoLine3 line2, LineExtension extension, Tolerance tolerance)
        {
            ...
            {
                // The first is a point: the nearest point of the second to it.
                s = 0.0;
                t = extendSecond ? f / e : Clamp(f / e);
            }
            else if (e <= squaredTolerance)
            {
                t = 0.0;
                s = extendFirst ? -c / a : Clamp(-c / a);
            }
            else
            {
                double denominator = a * e - b * b;

                // Lines a trillionth of a radian apart would put the crossing of their carriers out at
                // numbers that mean nothing; they are treated as the parallel lines they are to within
                // rounding, where any facing pair is as close as any other.
                bool parallel = denominator <= 1e-24 * a * e;

                if (extendFirst && extendSecond)
                {
                    s = parallel ? 0.0 : (b * f - c * e) / denominator;
                    t = (b * s + f) / e;
                }
                else if (extendFirst)
                {
                    // The distance over a free s and a clamped t is convex, so the best t is the free optimum
                    // clamped into the second segment, and s follows from it.
                    t = parallel ? 0.0 : Clamp((a * f - b * c) / denominator);
                    s = (b * t - c) / a;
                }
                else
                {
                    s = parallel ? 0.0 : Clamp((b * f - c * e) / denominator);
                    t = (b * s + f) / e;
                }
            }
