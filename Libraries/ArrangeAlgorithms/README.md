...
[PlaneGeometry](https://www.nuget.org/packages/PlaneGeometry/), which used to ship inside this package
and became one of its own in 3.0.0.

> **Moving from 2.x?** The geometric types moved to the `PlaneGeometry` package and gained a `2`
> suffix, and `Tolerance` moved to `CommonGeometry`. See
...
`PlaneGeometry` and `CommonGeometry` come with it as dependencies.
...
```bash
dotnet build Samples/ArrangeAlgorithms.CadTest/ArrangeAlgorithms.CadTest.csproj
```
...
2. Build the project:
   ```bash
   dotnet build Samples/ArrangeAlgorithms.TeklaTest/ArrangeAlgorithms.TeklaTest.csproj
   ```
3. Run the compiled executable:
   ```bash
   Samples/ArrangeAlgorithms.TeklaTest/bin/Debug/net48/ArrangeAlgorithms.TeklaTest.exe
   ```
